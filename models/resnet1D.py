...
class ResNet1D(nn.Module):
    ...
    def forward(self, x, inputLen, lenReq):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        ...
        xlist = [x[i, :, :inputLen[i]].transpose(1,0) for i in range(x.shape[0])]

        outputBatch = self.outpadding(list(xlist), inputLen, lenReq)

        return outputBatch, lenReq

    # def forward(self, x, inputLen, lenReq):
    #     hidden = []
    #     x = self.conv1(x)
    #     hidden.append(x.detach().clone().cpu())
    #     x = self.bn1(x)
    #     hidden.append(x.detach().clone().cpu())
    #     x = self.relu(x)
    #     hidden.append(x.detach().clone().cpu())
    ...
    #
    #     # import numpy as np
    #     # np.save('data_my.npy', outputBatch.detach().cpu().numpy())
    #
    #     return outputBatch, lenReq, hidden
